...
export class EmailConfirmationService {
    ...
    public async newVerification(req: Request, dto: ConfirmationDto) {
        ...
        if(!existingToken) {
            throw new NotFoundException('Confirm token was not found. Please, make sure that u have a right token')
        }

        const hasExpired = new Date(existingToken.expiresIn) < new Date()

        if(hasExpired) {
            throw new BadRequestException('The token times is expired.')
        }
